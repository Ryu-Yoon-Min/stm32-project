...
Ds18b20Sensor_t	*temperSensor = &ds18b20[0];
...
bool	Ds18b20_Init_Simple(void){
		...
		OneWire.ROM_NO[5] = 0x1;
		OneWire.ROM_NO[6] = 0x3c;
		OneWire.ROM_NO[7] = 0x67;

		OneWire_GetFullROM(&OneWire, temperSensor->Address);
		DS18B20_SetResolution(&OneWire, temperSensor->Address, DS18B20_Resolution_12bits);	// 12bit로 설정되어있었음. Resoulution : 해상도, 하나의 온도를 저장할 때 공간을 얼마나 사용할 것인가
		Ds18b20Delay(50);
...
bool	Ds18b20_Init(void)
{
	...
	do
	{
		OneWire_Init(&OneWire,_DS18B20_GPIO ,_DS18B20_PIN);
		TempSensorCount = 0;	
		while(HAL_GetTick() < 3000)	// sysTick : 시스템 가동 시 올라감
			Ds18b20Delay(100);
		OneWireDevices = OneWire_First(&OneWire); // *** OneWire_First에서 OneWire_Search()사용 ***
	...
	{
		Ds18b20Delay(50);
    DS18B20_SetResolution(&OneWire, ds18b20[i].Address, DS18B20_Resolution_12bits);	// 12bit로 설정되어있었음. Resoulution : 해상도, 하나의 온도를 저장할 때 공간을 얼마나 사용할 것인가
		Ds18b20Delay(50);
...
void checkConverting(){
	m_busy_line = 1;
	m_isConverting = !DS18B20_AllDone(&OneWire);	// DS18B20_AllDone(&OneWire) : 완료 1, 미완료 0 반환
	m_busy_line = 0;
}
