...
이 STM32CubeIDE 프로젝트는 DS18B20 온도 측정 경로와 4-Digit FND 갱신 경로를 독립적으로 분리한 데모입니다.

## 목적 (Purpose)

기존의 FND 디스플레이 로직은 메인 루프에서 모든 자릿수를 갱신하는 블로킹(Blocking) 함수를 사용했습니다. 이로 인해 DS18B20 온도 변환 로직이 실행될 때마다 FND 갱신 주기가 불규칙해져 디스플레이가 깜빡이는 플리커링(Flickering) 현상이 발생했습니다.

이 프로젝트는 FND 갱신 작업을 TIM3 하드웨어 타이머 인터럽트로 이동시켜 해당 문제를 구조적으로 해결합니다.
...
다음의 로컬/생성 파일들은 의도적으로 Git 추적에서 제외되었습니다 (`.gitignore` 적용):
