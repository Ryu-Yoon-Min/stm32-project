...
void OneWire_WriteByte(OneWire_t* OneWireStruct, uint8_t byte) {
	...
	while (i--) {
		/* LSB bit is first */
		OneWire_WriteBit(OneWireStruct, byte & 0x01);	// LSB를 읽음.
		byte >>= 1;	// 10110010 -> 01011001
	}
}
...
uint8_t OneWire_First(OneWire_t* OneWireStruct) {
	...
	return OneWire_Search(OneWireStruct, ONEWIRE_CMD_SEARCHROM);	// *** OneWire commands 존재 ***
}
...
uint8_t OneWire_Search(OneWire_t* OneWireStruct, uint8_t command) {
	...
	if (!OneWireStruct->LastDeviceFlag)
	{
		...
		do {
			...
			if ((id_bit == 1) && (cmp_id_bit == 1)) {		// 둘 다 1이면 break; 연결된 장치 없음
				break;
			} else {
